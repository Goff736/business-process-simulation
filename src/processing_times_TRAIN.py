...
def calculate_net_durations_optimized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep only the lifecycle transitions needed for net duration.
    WORK_TRANSITIONS = {"start", "resume", "suspend", "complete"}
    dfx = df.copy()

    # Normalise transition values once (lower-case strings).
    dfx["lifecycle:transition"] = (
        dfx["lifecycle:transition"].astype(str).str.strip().str.lower()
    )
    dfx = dfx[dfx["lifecycle:transition"].isin(WORK_TRANSITIONS)].copy()
    ...
    # Sort once, then do a single linear scan. This preserves the original
    # state-machine logic but avoids slow groupby+iterrows.
    dfx = dfx.sort_values(["case:concept:name", "time:timestamp"]).reset_index(drop=True)

    case_arr = dfx["case:concept:name"].astype(str).to_numpy()
    act_arr = dfx["concept:name"].astype(str).to_numpy()
    res_arr = dfx["org:resource"].fillna("").astype(str).to_numpy()
    tr_arr = dfx["lifecycle:transition"].astype(str).to_numpy()
    # For fast diffs: nanoseconds since epoch (UTC)
    ts_ns = dfx["time:timestamp"].astype("int64").to_numpy()

    records = []

    current_case = case_arr[0] if len(case_arr) else ""
    ...
    for i in range(len(case_arr)):
        ...
        if tr == "start" or tr == "resume":
            ...
            if working and current_activity == act and last_start_ns is not None:
                net_work_s += max(0.0, (ts_i - last_start_ns) / 1e9)
                working = False
                last_start_ns = None
                ...
                records.append(
                    {
                        ...
                        "net_duration_sec": float(net_work_s),
                    }
                )
