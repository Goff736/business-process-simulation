...
# Cache for CaseAttributeSampler so evaluation runs don't repeatedly parse the
# full CSV for each allocation configuration.
_CASE_ATTR_CACHE: Dict[str, Dict[str, Any]] = {}
...
class CaseAttributeSampler:
    ...
    def _load(self):
        ...
        cached = _CASE_ATTR_CACHE.get(cache_key)
        if cached is not None:
            self.loan_goals = cached["loan_goals"]
            self.loan_goal_p = cached["loan_goal_p"]
            self.app_types = cached["app_types"]
            self.app_type_p = cached["app_type_p"]
            self.amounts_global = cached["amounts_global"]
            self.global_lo = cached["global_lo"]
            self.global_hi = cached["global_hi"]
            self.amounts_by_goal = cached["amounts_by_goal"]
            self.bounds_by_goal = cached["bounds_by_goal"]
            self.amounts_by_goal_app = cached["amounts_by_goal_app"]
            self.bounds_by_goal_app = cached["bounds_by_goal_app"]
            return

        # Read only the case-level attribute columns we use.
        try:
            cols = list(pd.read_csv(self.csv_path, nrows=0).columns)
            wanted = [
                "case:LoanGoal",
            ...
            df = pd.read_csv(self.csv_path, usecols=usecols, low_memory=False)
        except Exception:
            return
        ...
        # Save to cache for subsequent SimulationEngine instances.
        _CASE_ATTR_CACHE[cache_key] = {
            "loan_goals": self.loan_goals,
            "loan_goal_p": self.loan_goal_p,
            "app_types": self.app_types,
            "app_type_p": self.app_type_p,
            "amounts_global": self.amounts_global,
            "global_lo": self.global_lo,
            "global_hi": self.global_hi,
            "amounts_by_goal": self.amounts_by_goal,
            "bounds_by_goal": self.bounds_by_goal,
            "amounts_by_goal_app": self.amounts_by_goal_app,
            "bounds_by_goal_app": self.bounds_by_goal_app,
        }
...
class SimulationEngine:
    ...
    def _allocate(self, t: pd.Timestamp):
        ...
        while True:
            ...
            free_set = set(available - self.busy_resources)
            if not free_set:
                return
            ...
            def try_queue(queue: Deque[Task], resumed: bool) -> bool:
                ...
                for _ in range(n):
                    ...
                    # Filter candidates by permission (fast set intersection)
                    allowed = (
                        self.permissions.get_allowed_resources_set(task.activity)
                        if hasattr(self.permissions, "get_allowed_resources_set")
                        else set(self.permissions.get_allowed_resources(task.activity))
                    )
                    candidates = list(free_set & allowed)
                    if not candidates:
                        queue.append(task)
                        continue
    ...
    def _allocate_batch(self, t: pd.Timestamp):
        ...
        # Collect a batch of at most batch_size tasks: suspended first (priority),
        # then waiting. Limiting the batch is essential — otherwise the cost
        # matrix can grow to thousands of tasks and blow up memory/time.
        batch: List[Task] = []
        for tk in self.suspended:
