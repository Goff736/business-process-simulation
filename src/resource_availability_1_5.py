...
def _load_and_repair_minimal(csv_path: str) -> pd.DataFrame:
    # Some datasets (e.g., PM4Py-exported CSV) don't include the legacy
    # "Action" column that appears in some raw BPI 2017 CSV variants.
    # We only need (resource, timestamp) for availability training.
    cols = list(pd.read_csv(csv_path, nrows=0).columns)
    usecols = ["org:resource", "time:timestamp"]
    if "Action" in cols:
        usecols = ["Action", "org:resource", "time:timestamp"]

    df = pd.read_csv(csv_path, usecols=usecols, low_memory=False)

    # BPI 2017 raw CSV can have rows where columns collapse into "Action".
    # If "Action" is not present, we assume the CSV is already clean.
    if "Action" in df.columns:
        broken = (
            df["org:resource"].isna()
