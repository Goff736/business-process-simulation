...
class PermissionsModel:
    ...
    def __init__(
        ...
        *,
        ...
        # Fast lookup: activity -> set(resources)
        # This avoids repeatedly scanning all resources in advanced mode
        # and speeds up the simulation dispatch loop.
        self._activity_allowed_sets: Dict[str, Set[str]] = self._build_activity_allowed_sets()

    def _build_activity_allowed_sets(self) -> Dict[str, Set[str]]:
        if self.mode == "basic":
            return {
                act: set(map(str, (res_list or [])))
                for act, res_list in (self.artifact.activity_to_resources or {}).items()
            }

        # advanced: invert resource -> permissions into activity -> resources
        inv: DefaultDict[str, Set[str]] = defaultdict(set)
        for r, perms in (self.artifact.resource_expanded_perms or {}).items():
            r = str(r)
            for act in perms:
                inv[str(act)].add(r)
        return dict(inv)
    ...
    def get_allowed_resources_set(self, activity: str) -> Set[str]:
        """Return the set of allowed resources for an activity (cached)."""
        return self._activity_allowed_sets.get(str(activity), set())
    ...
    def get_allowed_resources(self, activity: str) -> List[str]:
        ...
        return sorted(self.get_allowed_resources_set(activity))

    def can_execute(self, resource: str, activity: str) -> bool:
        resource = str(resource)
        activity = str(activity)
        return resource in self.get_allowed_resources_set(activity)